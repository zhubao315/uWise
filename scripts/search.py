        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                if response.status == 200:
                    ...
                    logger.error(f"搜索失败: {query}, 状态码: {response.status}, 错误: {error_text}")
                    return []
...
def search_tavily(query: str, api_key: str) -> List[Dict]:
    ...
    Returns:
        搜索结果列表
    """
    return asyncio.run(search_tavily_async(
        query=query,
        api_key=api_key,
...
async def batch_search(
    ...
    tasks = [
        search_tavily_async(query, api_key, search_depth, max_results, timeout)
